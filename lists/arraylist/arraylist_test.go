...
func TestListAny(t *testing.T) {
	list := New[string]()
	list.Add("a", "b", "c")
	isAny := list.Any(func(index int, value string) bool {
		return value == "c"
	})
	if isAny != true {
...
func TestListSerialization(t *testing.T) {
	...
	assert := func() {
		values := list.Values()
		if actualValue, expectedValue := fmt.Sprintf("%s%s%s", values[0], values[1], values[2]), "abc"; actualValue != expectedValue {
			t.Errorf("Got %v expected %v", actualValue, expectedValue)
		}
